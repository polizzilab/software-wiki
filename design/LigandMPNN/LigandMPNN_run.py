...
def main(args) -> None:
    ...
    if not args.overwrite:
        ...
        for pdb_path in pdb_paths:
            output_fasta = base_folder + "/seqs/" + Path(pdb_path).stem + args.file_ending + ".fa"
            if Path(output_fasta).exists():
                exists.append(pdb_path)
            else:
                not_exists.append(pdb_path)
        if len(not_exists) == 0:
            print('Nothing to do; all outputs exist already: ', exists)
            return
        print('Skipping these pdb files since the outputs already exist:', exists)
        print('Running these pdb files:', not_exists)
        pdb_paths = not_exists
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        action='store_true',
        default=False,
        help="Flag to overwrite existing files. Default is to not",
    )
