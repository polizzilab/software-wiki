...
class Topology:
    ...
    def set_surface_res(self, alpha=9, selection_only=False):
        ...
        if selection_only:
            exp, inter, bur = partition_res_by_burial_old(self.pdb_ala_sel, alpha=alpha)
            rns = self.pdb_ala_sel.select('name CB or (resname GLY and name CA)').getResnums()
        else:
            exp, inter, bur = partition_res_by_burial_old(self.pdb_ala, alpha=alpha)
    ...
    def set_contacts(self, calpha_distance=10.5):
        ...
        as a pair of C alpha atoms with a distance less than 10.5 angstroms."""
